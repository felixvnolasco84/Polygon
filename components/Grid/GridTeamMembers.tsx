...
const GridTeamMembers = ({ items }: any) => {
  return (
    <div className="flex flex-col gap-8 px-4 lg:gap-12 lg:px-12 xl:px-24">
      <h4 className={`${neueLight.className} text-2xl lg:text-5xl xl:text-7xl leading-none`}>
        Conoce al equipo
      </h4>
        ...
        {items.map((item: any, index: any) => (
          <div className="flex flex-col gap-2 lg:gap-4 xl:gap-6" key={index}>
            <Image
              src={item.img}
              alt=""
              className="min-h-[145px] w-full rounded-[10px] bg-gray-500 object-cover object-center lg:max-w-none xl:h-[371px] xl:w-[313px]"
              width={313}
              height={371}
            />
